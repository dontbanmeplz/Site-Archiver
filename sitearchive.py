...
def crawler(startpage, maxpages=100, singledomain=True):
    ...
    while pages < maxpages and pagequeue:
        ...
        except (requests.exceptions.MissingSchema, requests.exceptions):
            print("*FAILED*:", url)
            failed += 1
            continue
...
if __name__ == "__main__":
    wa = pyfiglet.figlet_format("WEB ARCHIVER")
    print(wa)
    print('Credits: ')
    print('Main framework : Doug Mahugh')
    print('Archiver : akamhy')
    print('ASCII : pyfiglet')
    print("what page do you want to save?")
    print("-----------------------------")
    urllong = input('>>>>> ')
    sys.stdout = open(sys.stdout.fileno(), mode="w", encoding="utf-8", buffering=1)
    START = default_timer()
    crawler(urllong, maxpages=1000)
    END = default_timer()
